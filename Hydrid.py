...
if __name__ == "__main__":
    ...
    for count in ray_counts:
        ...
        for i, d in enumerate(directions):
            dist, checks = scan_hybrid(SCANNER_POS[0], SCANNER_POS[1], SCANNER_POS[2], d)
            tot_checks += checks
    ...
    df.to_csv("3_hybrid_results.csv", index=False, encoding="utf-8-sig")

    fig, axes = plt.subplots(1, 3, figsize=(15, 4))
    fig.suptitle("Hybrid Optimization (Fast AND Accurate)", fontsize=14, fontweight='bold')
    
    axes[0].plot(df["Ray Count"], df["Time(ms)"], marker='o', color='green')
    axes[0].set_title("Time(ms) - Very Fast")
    
    axes[1].plot(df["Ray Count"], df["Avg Checks"], marker='o', color='green')
    axes[1].set_title("Avg Checks - Very Low")
    
    axes[2].plot(df["Ray Count"], df["Avg Error"], marker='o', color='green')
    axes[2].set_title("Avg Error - Perfect (0)")

    plt.tight_layout()
    plt.savefig("3_hybrid_graph.png", dpi=300)
    print("완료! 결과 저장됨: 3_hybrid_results.csv, 3_hybrid_graph.png")
